...
Dim AdjustBy As Double
Dim LastSam As Long
Dim LevelDB As Double

'   Note: all wave forms are preset to 4000 milliseconds (4 seconds)
'   The release timing is based on that duration backwards

    LastSam = (bRate * (AttackMS / 1000))
    If (LastSam > UBound(Wave(Which).Sample)) Then LastSam = UBound(Wave(Which).Sample)

    If (LastSam > 0) Then

'   AttackMS -> PeakDB = the time it takes to go from zero to PeakDB

        For eIndx = LastSam To 0 Step -1
            AdjustBy = (AttenuationValue(PeakDB * (eIndx / LastSam))) / ((2 ^ bWidth) - 1)
...
'   PeakDB -> DecayMS - SustainDB = the time to acheive SustainDB
    
    If (LastSam > vIndx) Then
    
        For eIndx = vIndx To LastSam
            If (eIndx > UBound(Wave(Which).Sample)) Then Exit Sub
            LevelDB = (PeakDB - (PeakDB - SustainDB))
            AdjustBy = (AttenuationValue(LevelDB * ((eIndx - vIndx) / (LastSam - vIndx)))) / ((2 ^ bWidth) - 1)
            Wave(Which).Sample(eIndx) = (Wave(Which).Sample(eIndx) * AdjustBy)
        Next
...
'   SustainDB -> ReleaseMS = the time it takes to close the envelope
    
    If (LastSam > vIndx) Then
    
        For eIndx = LastSam To vIndx Step -1
            AdjustBy = (AttenuationValue(SustainDB * ((eIndx - vIndx) / (LastSam - vIndx)))) / ((2 ^ bWidth) - 1)
            Wave(Which).Sample(eIndx) = (Wave(Which).Sample(eIndx) * AdjustBy)
        Next
